...
DEFAULT='Start the trading day. Read BRIEF.md and OPERATIONS.md, then MEMORY.md and every memory file it links. Run node scripts/preflight.js and fix anything it flags. CHECK THE CALENDAR FIRST: search for todays scheduled US economic releases (CPI, PPI, PCE, NFP/jobs, FOMC decision or minutes, retail sales, GDP, ISM) and any Fed speakers, with their times in PST. State what you found. Treat the 15 minutes either side of a major release as a no-entry window, and do not carry a new entry into an FOMC decision. If you cannot determine the calendar, say so explicitly rather than assuming the day is clear. Set the chart to MNQ 5m. PLANNING FLAGS: the Pivot indicator's in_3 (includeToday) and in_4 (liveInvalidate) are left TRUE overnight so the level set reflects the coming session. Read the level map with them TRUE, then set BOTH BACK TO FALSE before the open — a live session must not run with the planning flags on. Verify the applied values after changing them; do not assume the write took. Do not save the layout (it is shared with Zaid's laptop) unless he asks. Build the pre-market plan, then run the session on a Monitor tick loop per OPERATIONS.md, stating a heartbeat every wake-up. Follow every rule in BRIEF.md. Do not trade blind and do not weaken a guardrail.'
MSG="${1:-$DEFAULT}"
