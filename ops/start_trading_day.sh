...
DEFAULT='Start the trading day. Read BRIEF.md and OPERATIONS.md, then MEMORY.md and every memory file it links. Run node scripts/preflight.js and fix anything it flags. CHECK THE CALENDAR FIRST: search for the scheduled US economic releases today (CPI, PPI, PCE, NFP/jobs, FOMC decision or minutes, retail sales, GDP, ISM) and any Fed speakers, with times in PST. State what you found. Treat the 15 minutes either side of a major release as a no-entry window, and do not carry a new entry into an FOMC decision. If you cannot determine the calendar, say so explicitly rather than assuming the day is clear. Set the chart to MNQ 5m. PLANNING FLAGS: in_3 (includeToday) and in_4 (liveInvalidate) on the Pivot indicator are left TRUE overnight so the level set reflects the coming session. Read the level map with them TRUE, then set BOTH BACK TO FALSE before the open - a live session must not run with the planning flags on. Verify the applied values after changing them; do not assume the write took. Do not save the layout (it is shared with the laptop) unless asked. Build the pre-market plan, then run the session on a Monitor tick loop per OPERATIONS.md, stating a heartbeat every wake-up. Follow every rule in BRIEF.md. Do not trade blind and do not weaken a guardrail. LEARNING IS PART OF THE JOB, per OPERATIONS.md section 10: append each trade outcome to the trading-day-log memory AS IT HAPPENS, not only at the close, because a session that dies at 11:00 loses everything it learned that morning. After the 12:55 flatten, write the day block, then ask explicitly whether anything today changed what you believe - and if so write a NEW memory file with a Why and a How to apply, and add it to MEMORY.md. Correct or delete any memory that today proved wrong. State in your day summary which memory files you wrote or changed.'
MSG="${1:-$DEFAULT}"
